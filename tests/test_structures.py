"""kabinet's structures: what it can send across a node boundary, and how.
...
from kabinet import arkitekt as declared  # noqa: E402
...
def test_merging_stamps_the_service_that_owns_them() -> None:
    ...
    app = AppRegistry()
    app.merge(declared.registry, service="kabinet")

    services = {s.service for s in app.structure_registry.structures()}
    assert services == {"kabinet"}
