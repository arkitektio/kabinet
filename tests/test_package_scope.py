"""kabinet must stay importable, and correct, without arkitekt.

arkitekt is an optional integration. Two places may know about it: the integration
module ``kabinet/arkitekt.py``, and ``kabinet/__init__.py``, which imports that module
...
PACKAGE = Path(kabinet.__file__).parent
...
def test_only_the_integration_module_imports_arkitekt() -> None:
    ...
    assert not offenders, (
        f"{offenders} import arkitekt at module scope. kabinet works without arkitekt: "
        "take a reference (the client) instead of importing it."
    )


def test_kabinet_imports_with_arkitekt_unavailable() -> None:
    ...
    script = (
        ...
        "assert hasattr(Kabinet, 'aget_pod')\n"
        "print('ok')\n"
    )
